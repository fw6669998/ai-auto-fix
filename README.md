...
> 还在手动排查线上错误、逐行 Review 代码？**AI Auto Fix** 是一套基于 Flask 的自动化系统，它接收错误日志和 Git Commit 信息，调度 Claude Code Agent 在隔离 worktree 中自动完成代码修复与审查，并通过企业微信实时推送结果。开发只需关注业务，把重复劳动交给 AI。
...
*Commit 审查记录：清晰展示每条 Commit 的审查结果——通过、有问题、跳过。发现问题后，系统会自动创建 `ai-review/xxx` 分支复核。*
...
*企业微信通知示例：包含项目名、修复状态、错误类型、造成原因、修复方法以及直达详情的链接。*
...
- **可扩展提示词**：Agent 的修复与审查逻辑统一收口在 `src/prompts.py`。
...
AI Auto Fix 不是替代开发者，而是把**重复、机械、低价值**的排查与修补工作自动化，让人类工程师把时间花在真正需要创造力的设计上。如果你也正在寻找一种方式，把 Claude Code 的能力无缝接入日常开发流程，不妨试试 AI Auto Fix。
